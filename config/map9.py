...
from config.scenario.robot_start_pos_init_2D_9 import robot_init_2d
...
n_robot = 5     # mode
n_obs = 2       # here we only have the 0, 1, 3 num of the circle obs
...
OBSTACLE_DATABASE = {
    ...
    'obs_config': rectangle_obs_init_2d(0)
}
...
def initialize_system_and_visualizer(visualizer_init: bool=True):
    ...
    if visualizer_init:
        ...
        for i in multi_robot_keys:
            ...
            if robot_init_data['robot_type'] == 'passive':
                visualizer.add_robot(i, robot_init_data['pos_real'], robot_init_data['radius'], show_orientation=False, color=(0, 0, 0))
            else:
